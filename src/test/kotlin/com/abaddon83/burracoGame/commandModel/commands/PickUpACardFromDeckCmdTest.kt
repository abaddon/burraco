...
class PickUpACardFromDeckCmdTest: KoinTest {
    ...
    @Test
    fun `(async) Given a player during its turn, when I receive the comand to pick up a card, then the card is picked-up`(){
    ...
    @Test
    fun `Given a player during its turn, when I receive the comand to pick up a card, then the card is picked-up`(){
        val command = PickUpACardFromDeckCmd(gameIdentity = gameIdentity,playerIdentity = playerIdentity1)
