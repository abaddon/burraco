...
import com.abaddon83.burracoGame.controller.adapter.messages.requests.DropCardRequest
import com.abaddon83.burracoGame.controller.adapter.messages.requests.JoinGameRequest
import com.abaddon83.burracoGame.controller.adapter.messages.requests.StartGameRequest
import com.abaddon83.burracoGame.controller.adapter.messages.BurracoGameModule
import com.abaddon83.burracoGame.controller.adapter.messages.CardsModule
    ...
    route("games/burraco/{gameId}") {
        ...
        route("join") {
            post {
                ...
                val game = commandController.joinPlayer(burracoGameIdentity = gameIdentity, playerIdentity = PlayerIdentity(joinGameRequest.playerIdentity))
        ...
        route("start") {
            post {
                ...
                val game = commandController.startGame(burracoGameIdentity = gameIdentity, playerIdentity = PlayerIdentity(startGameRequest.playerIdentity))
                call.respond("")//BurracoGameModule.from(game))
            }
        }
        route("players") {
            route("{playerId}") {
                ...
                route("pickupcard") {
                    post {
                        val gameIdentity = requireNotNull(GameIdentity.create(call.parameters["gameId"]!!)) { "PlayerIdentity ${call.parameters["gameId"] ?: "null"} missing or malformed" }
                        val playerIdentity = requireNotNull(PlayerIdentity.create(call.parameters["playerId"]!!)) { "PlayerIdentity ${call.parameters["playerId"] ?: "null"} missing or malformed" }
                        val game = commandController.pickUpCardFromDeck(burracoGameIdentity = gameIdentity, playerIdentity = playerIdentity)
                        call.respond("")//BurracoGameModule.from(game))
                    }
                }
                route("dropcard") {
                    post {
                        ...
                        val game = commandController.dropCardOnDiscardPile(burracoGameIdentity = gameIdentity, playerIdentity = playerIdentity, cardToDrop = cardToDrop)
                        call.respond("")//BurracoGameModule.from(game))
                    }
                }
            }
